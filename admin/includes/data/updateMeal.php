...
if(isset($_POST["update_meal"])) {
    ...
    $meal_id = htmlspecialchars($_POST["meal-id"]);
...
//            header("Location: ../../add_meal.php");
    ...
    $mealQuery = "UPDATE food_items SET name=?, price=?, description=?, images=?, meal_type=?, meal_chef=?, meal_ingredients=?, meal_place=? WHERE id=?";
    $insertStmt = $db->prepare($mealQuery);
    $insertStmt->bind_param("sdssssssd", $meal_name, $meal_price, $meal_description, $meal_image, $meal_type, $meal_chef, $meal_ingredients, $meal_place, $meal_id);
    $insertStmt->execute();
